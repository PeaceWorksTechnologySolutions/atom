...
class sfEcommercePluginAddObjectToCartComponent extends sfComponent
{
  public function execute($request)
  {
    ...
    if ($this->may_disseminate && $this->resource->digitalObjectsRelatedByobjectId[0]) {
  ...
  public function right_is_allowed($default) 
  {
    ...
    foreach ($ancestors as $item) {
        ...
        $results = QubitGrantedRight::getByObjectIdAndAct($item->id, QubitTerm::RIGHT_ACT_DISSEMINATE_ID);
        foreach ($results as $granted_right) {
            return $granted_right->restriction;
        }
    }
    return $default;
  }
  ...
  public function get_resolution() {
    $filepath = $this->resource->digitalObjectsRelatedByobjectId[0]->getAbsolutePath();
