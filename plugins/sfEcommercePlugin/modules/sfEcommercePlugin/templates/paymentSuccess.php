...
<?php slot('content') ?>
    ...
    <form name="paypalform" action="<?php echo sfConfig::get("ecommerce_paypal_url") ?>" 
      ...
      <?php if (!empty($phone)) { ?>
        <input type="hidden" name="night_phone_a" value="<?php echo substr($phone, 0, 3) ?>"/>
        <input type="hidden" name="night_phone_b" value="<?php echo substr($phone, 3, 3) ?>"/>
        <input type="hidden" name="night_phone_c" value="<?php echo substr($phone, 6, 4) ?>"/>
      <?php } ?>
